...
const HOURS = Array.from({ length: 24 }, (_, i) => i + 0); // 7 AM to 7 PM
...
/**
 * CalendarGrid component displays a weekly calendar grid with time slots and appointments.
 *
 * @component
 * @param {Object} props - Component props.
 * @param {Date} props.currentDate - The currently selected date to display the week for.
 * @param {Array<Object>} props.appointments - List of appointment objects to display in the calendar.
 * @param {Array<Object>} props.participants - List of participant objects, each with at least an `id`, `name`, and `color`.
 * @param {function(Date): void} props.onDateChange - Callback fired when the user navigates to a different week.
 * @param {function(Object): void} props.onAppointmentClick - Callback fired when an appointment is clicked.
 * @param {function(Date, string): void} props.onTimeSlotClick - Callback fired when a time slot is clicked, receives the date and time string.
 *
...
export default function CalendarGrid({ 
  ...
  const appointmentsByDayAndTime = useMemo(() => {
      return days.map((day) => {
          ...
          for (let i = 0; i < TIME_SLOTS.length; i += 6) {
              chunkedTimeSlots.push(TIME_SLOTS.slice(i, i + 6));
          }
          ...
          return {
              ...
              timeSlots: chunkedTimeSlots.map((slotGroup) => {
                  ...
                  slotGroupStartDateTime.setHours(
                  ...
                  slotGroupEndDateTime.setHours(
                      parseInt(endHour, 10),
                      parseInt(endMinute, 10) + 30,
                      0,
                      0
                  );
                  ...
                  const relevantAppointments = appointments.filter((appointment) => {
                      ...
                      appointmentStart.setHours(
                          parseInt(appHour, 10),
                          parseInt(appMinute, 10),
                          0,
                          0
                      );

                      return (
                          appointmentStart >= slotGroupStartDateTime &&
                          appointmentStart < slotGroupEndDateTime
                      );
                  });
